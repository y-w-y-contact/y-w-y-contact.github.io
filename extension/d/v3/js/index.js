...
async function ywy_download(ywy_file_json, this_player_type) {
    ...
    if (this_player_type == "bangumi" || this_player_type == "video") {
        ...
        let this_opt_temp =  ffmpeg.getLastOutput();
        let this_audio_rate_match = this_opt_temp.match(/Audio: .+?, (\d+) Hz/);
...
async function ywy_console() {
    ...
    if (!ywy_has_file_parameter) {
        ...
        if (ywy_file_json.type == "bangumi" || ywy_file_json.type == "video") {
            ...
            if (ywy_file_size_sum >= 1932730000) {
                document.getElementById("ywy_media_size").innerText += " (檔案大於1.8 GB，基於瀏覽器限制，請使用電腦版下載，避免發生問題)";
            }
            //填入基本訊息結束//
